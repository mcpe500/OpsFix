...
class OpsFixSupabaseUser extends BaseAuthUser {
  OpsFixSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> opsFixSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = OpsFixSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
